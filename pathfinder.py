...
def update_visitors(display, grid, visitors):
  update_display()
  rect_width = (WIDTH - 1) / cols
  rect_height = (HEIGHT - 21) / cols
...
def pathfind():
  if not start or not end:
    ...
    return
  # path, visitor = bfs(start, end)
  # path, visitor = dfs(start, end)
  path, visitor = a_star(start, end)
  update_visitors(display, grid, visitor)
  if not path:
